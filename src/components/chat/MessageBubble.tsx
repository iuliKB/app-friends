...
import { SystemMessageRow } from '@/components/chat/SystemMessageRow';
import { ChatTodoBubble } from '@/components/chat/ChatTodoBubble';
...
interface MessageBubbleProps {
  ...
  // Phase 29.1 Plan 07 additions — chat-to-do data passed in by ChatRoomScreen:
  chatTodoList?: ChatTodoList | null;
  chatTodoItems?: ChatTodoItem[];
  chatTodoAssigneeName?: string;
  onCompleteChatTodoItem?: (itemId: string) => Promise<unknown>;
}
    ...
    >
      <View style={[quotedBlockStyles.quotedBlock, { backgroundColor: quotedBg }]}>
        ...
        <View style={quotedBlockStyles.quotedContent}>
          ...
          <Text
            style={[quotedBlockStyles.quotedPreview, { color: previewColor }]}
            numberOfLines={1}
          >
            {previewText}
          </Text>
...
export function MessageBubble({
  ...
  chatTodoList = null,
  chatTodoItems = [],
  chatTodoAssigneeName = '',
  onCompleteChatTodoItem,
}: MessageBubbleProps) {
  ...
  function handleLongPress(event: { nativeEvent: { pageY: number } }) {
    ...
    // Phase 29.1 Plan 07 (Pitfall 9): system + to-do bubbles have no context menu for v1.
    if (isSystem) return;
    if (isTodo) return;
  ...
  // Phase 29.1 Plan 07 — system + to-do bubble dispatch flags (Pitfall 10).
  const isSystem = message.message_type === 'system';
  const isTodo = message.message_type === 'todo';
  ...
  // Phase 29.1 Plan 07 — system + to-do branches must come BEFORE isPoll and the
  // isOwn/others dispatch (Pitfall 10). System messages must always render
  // centered regardless of sender_id matching the current user; to-do bubbles
  // are dispatched to ChatTodoBubble (which is responsible for the assignee-
  // gated checkbox semantics).
  if (isSystem) {
  ...
  if (isTodo) {
    return (
      <ChatTodoBubble
        ...
        list={chatTodoList}
        items={chatTodoItems}
        assigneeName={chatTodoAssigneeName}
        onCompleteItem={onCompleteChatTodoItem ?? (async () => undefined)}
      />
    );
  }
              ...
              {isImage ? (
                <TouchableOpacity
                  ...
                  accessibilityLabel={
                    message.pending
                      ? 'Sending photo...'
                      : `Photo from ${message.sender_display_name}`
                  }
                  style={{ padding: 0 }}
                >
