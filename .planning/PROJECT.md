...
---
*Last updated: 2026-05-13 — Phase 31 complete (Adopt TanStack Query for server-state caching and cross-screen reactivity: 31 hooks migrated across 8 waves, 5 ref-counted Supabase Realtime subscribers in realtimeBridge, AsyncStorage persistence with selective dehydration, mutationShape regression gate enforcing canonical Pattern 5 across 17 files / 21 mutation blocks, authBridge clears cache + useStatusStore on SIGNED_OUT, src/hooks/README.md boundary doc). 8/8 must-haves verified; full jest suite 243/243 green; all three final smokes PASS. Deferred to follow-up phase: chat-list message-preview reactivity regression + pre-existing in-chat widget reactivity gaps (polls, todo lists, wishlist, image attachments). v1.8 milestone is now feature-complete — all 8 v1.8 phases shipped.*
