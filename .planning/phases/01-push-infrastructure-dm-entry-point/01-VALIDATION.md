...
| 1-03-01 | 03 | 1 | PUSH-06, PUSH-07 | — | notifications-init.ts exists at module scope; categories + four Android channels registered; handler set with shouldShowBanner/shouldShowList | static | `npx tsc --noEmit && grep -q 'setNotificationCategoryAsync' src/lib/notifications-init.ts` | ❌ W0 (file is created in this task) | ⬜ pending |
| 1-04-01 | 04 | 4 | PUSH-01, PUSH-02, PUSH-05, PUSH-08 | T-1-02 (registration bypass), T-1-04 (permission spam) | session-ready useEffect in (tabs)/_layout.tsx calls registerForPushNotifications on auth + AppState 'active'; renders PrePromptModal when result is 'needs_pre_prompt'; Accept chains to native prompt, Decline sets opt-out | static | `grep -q "AppState.addEventListener" src/app/(tabs)/_layout.tsx && grep -q "PrePromptModal" src/app/(tabs)/_layout.tsx && grep -q "needs_pre_prompt" src/app/(tabs)/_layout.tsx && npx tsc --noEmit` | ❌ W0 | ⬜ pending |
| 1-05-01 | 05 | 2 | PUSH-03, PUSH-04 | T-1-03 (stale token retention) | usePushNotifications.ts upserts to (user_id, device_id); returns 'needs_pre_prompt' when eligible but pre-prompt not seen; off-toggle deletes server row; on-toggle re-registers silently | static | `npx tsc --noEmit && grep -q "device_id" src/hooks/usePushNotifications.ts && grep -q "needs_pre_prompt" src/hooks/usePushNotifications.ts && grep -q "campfire:push_pre_prompt_seen" src/hooks/usePushNotifications.ts` | ❌ W0 | ⬜ pending |
| 1-06-01 | 06 | 3 | PUSH-08 | T-1-04 (permission spam) | PrePromptModal component uses D-02 copy + @/theme tokens; useStatus and useFriends call markPushPromptEligible in updateStatus, updateContextTag, sendRequest, acceptRequest success paths | static | `grep -q "campfire:push_prompt_eligible" src/components/notifications/PrePromptModal.tsx && grep -q "markPushPromptEligible" src/hooks/useStatus.ts && grep -q "markPushPromptEligible" src/hooks/useFriends.ts && npx eslint src/components/notifications/PrePromptModal.tsx --max-warnings 0` | ❌ W0 | ⬜ pending |
| 1-07-01 | 07 | 2 | PUSH-05, PUSH-09 | T-1-03, T-1-05 (silent delivery failure) | notify-plan-invite filters invalidated_at IS NULL; sends with channelId 'plan_invites'; parses ticket-level errors; marks DeviceNotRegistered tokens invalidated | static | `grep -q "invalidated_at" supabase/functions/notify-plan-invite/index.ts && grep -q "DeviceNotRegistered" supabase/functions/notify-plan-invite/index.ts && grep -q "channelId: 'plan_invites'" supabase/functions/notify-plan-invite/index.ts` | ❌ W0 | ⬜ pending |
| 1-08-01 | 08 | 1 | DM-01 | — | HomeFriendCard wrapped in Pressable; onPress calls get_or_create_dm_channel + routes to /chat/room?dm_channel_id=...; onLongPress opens action sheet with View profile + Plan with... | static | `grep -q "Pressable" src/components/home/HomeFriendCard.tsx && grep -q "get_or_create_dm_channel" src/components/home/HomeFriendCard.tsx && grep -q "showActionSheet" src/components/home/HomeFriendCard.tsx` | ❌ W0 | ⬜ pending |
| 1-09-01 | 09 | 1 | DM-01 | — | plan-create accepts ?preselect_friend_id query param and pre-populates picker | static | `grep -q "preselect_friend_id" src/screens/plans/PlanCreateModal.tsx && npx tsc --noEmit` | ❌ W0 | ⬜ pending |
| 1-10-01 | 10 | 5 | PUSH-01..10, DM-01 | — | Manual smoke-test checklist signed off on EAS dev build (token registration, plan-invite delivery, action sheet, DM tap, toggle off/on, foreground re-register, ticket-error mark-invalidated) | manual | Smoke checklist file exists with all items ticked | N/A | ⬜ pending |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky*

**Sampling continuity check:** No three consecutive tasks lack automated verification. Tasks 1-01-01 (manual EAS build) and 1-10-01 (manual smoke test) are bookends; every task between has a `static` or `sql` verification command.
